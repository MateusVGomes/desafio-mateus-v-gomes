...
class CaixaDaLanchonete {
  ...
  calcularValorDaCompra(metodoDePagamento, itens) {
    ...
    if (this.mensagem === "") {
      const carrinho = itens.map((str) => {
        ...
        if (this.mensagem === "") {
          return this.validarProdutoEQuantidade(produto, quantidade) === ""
            ? this.converterParaCardapio(produto, quantidade)
            : "";
        }
      });
    ...
    return this.mensagem !== ""
      ? this.mensagem
      : this.definirValorFinal(metodoDePagamento);
  }
  ...
  validarProdutoEQuantidade(produto, quantidade) {
    return (this.mensagem = !this.cardapio.some(
      (item) => item.codigo === produto
    )
      ? "Item inválido!"
      : quantidade <= 0
  ...
  validarPratoPrincipal(carrinho) {
    for (let i = 0; i < carrinho.length; i++) {
      if (carrinho[i].extra === true) {
        let pratoPrincipal = !carrinho[i].dependencia.some((dep) =>
          carrinho.some((obj) => obj.codigo === dep)
        )
  ...
  converterParaCardapio(produto, quantidade) {
    const produtoCardapio = this.cardapio.find(
      (item) => item.codigo === produto
    );
  ...
  definirValorFinal(metodoDePagamento) {
    ...
    metodoDePagamento === "credito"
      ? (valorFinal = this.valorTotal + this.valorTotal * 0.03)
      : metodoDePagamento === "dinheiro"
      ? (valorFinal = this.valorTotal - this.valorTotal * 0.05)
      : (valorFinal = this.valorTotal);
